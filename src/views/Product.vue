  ...
  <div class="about sub-top-div" style="background: #000000;">
    ...
    <div class="section1">
      <div style="max-width: 1200px;margin: 0 auto;">
        <h1 v-if="this.$store.getters.getShowMethod == 1" class="page-title">
          豪克轮胎
        </h1>
        ...
        <Row class="about-row">
          ...
          >
            <Menu
              ...
              @on-select="changeSideSelect"
              @on-open-change="openSide"
            >
              <!--              <MenuItem v-for="(item,index) in groupDataList" :name="item.pgroupId">{{item.name}}</MenuItem>-->
              <Submenu
                v-for="(item, index) in groupDataList"
                :name="item.productGroup.pgroupId"
              >
                <template slot="title">
                  {{ item.productGroup.name }}
                </template>
                <MenuItem
                  :name="item2.tag.tagId"
                  v-for="(item2, index2) in item.pgroupTags"
                  style="display: flex;flex-direction: row;justify-content: center"
                >
                  <span
                    class="yuan"
                    v-show="item2.tag.tagId == activeName"
                  ></span>
                  {{ item2.tag.tagName }}
                </MenuItem>
              </Submenu>
          ...
          >
            ...
            <div v-else style="text-align: left">
              <Breadcrumb
                v-if="this.$store.getters.getShowMethod == 1"
                class="product-breadcrumb-item"
                separator=">"
              >
                <BreadcrumbItem>豪克轮胎</BreadcrumbItem>
                <BreadcrumbItem
                  ><a @click="backToPGroupAction">{{
                    tagName
                  }}</a></BreadcrumbItem
                >
                <BreadcrumbItem
                  v-if="typeof currentItem.product == 'undefined'"
                  >{{ currentItem.name }}</BreadcrumbItem
                >
              ...
              <template>
                ...
                >
                  <img
                    v-img:group
                    v-for="(item, index) in currentItem.detailImg.split(',')"
                    :src="item"
                    style="width: 100%"
                  />
                </div>
                <div style="width: 100%;" v-else>
                  <img
                    v-img:group
                    v-for="(item, index) in currentItem.product.detailImg.split(
                      ','
                    )"
                    :src="item"
                    style="width: 100%"
...
export default {
  ...
  mounted () {
    if (
      typeof this.$route.query.pgroupId !== 'undefined' &&
      this.$route.query.pgroupId !== null &&
      this.$route.query.pgroupId.length !== 0
    ) {
  ...
  methods: {
    ...
    getProductGroups () {
      ...
      //   websiteId: '4'
      // }
      const res = { data: projectGroupData }
      ...
      if (res.data.code !== 0) {
        ...
        if (res.data.data.length !== 0) {
          ...
          if (this.isDetail === 1) {
            ...
            if (
              typeof _this.activeName === 'undefined' ||
              _this.activeName.length == 0
            ) {
...
#product-menu.ivu-menu-vertical
